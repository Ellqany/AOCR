...
from attentionocr import Vectorizer
...
class DataGenerator:
    ...
    def csv_data_source(self, vectorizer: Vectorizer, directory: str, filename: str, is_training: bool = False, sep: str = ';'):
        '''
        ...
        return (len(examples), partial(self.examples_generator, examples=examples, vectorizer=vectorizer, is_training=is_training))

    def examples_generator(self, examples: list, vectorizer: Vectorizer, is_training: bool):
        '''
            convert the dataset to tensor ie (convert the image to tensor and the responding text in each image to tensor)
