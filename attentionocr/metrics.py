...
def fan_loss(y_true, y_pred, attention_weights, attention_target, ratio: float = 0.01) -> tf.Tensor:
    ...
    attn_loss = tf.losses.binary_crossentropy(
        attention_weights, attention_target)
    attn_loss = tf.reduce_sum(
        attn_loss * attn_char_mask * attn_sample_mask) / tf.reduce_sum(attn_char_mask)
